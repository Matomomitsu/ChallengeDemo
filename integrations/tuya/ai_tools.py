...
_SPACE_CONTEXT_TTL_SECONDS = int(os.getenv("TUYA_SPACE_CACHE_TTL", "300"))
_SCENE_BUILDER_CACHE: Dict[str, Dict[str, Any]] = {}
_SPACE_CONTEXT_CACHE: Dict[str, Dict[str, Any]] = {}
...
def _build_space_payload(space_id: str, devices: List[Any], scenes: List[Dict[str, Any]]) -> Dict[str, Any]:
    return {
        "space_id": space_id,
...
def _get_cached_space_context(space_id: str) -> Optional[Dict[str, Any]]:
    now = time.monotonic()
    with _CONTEXT_CACHE_LOCK:
        cached = _SPACE_CONTEXT_CACHE.get(space_id)
    if cached and (now - cached.get("timestamp", 0.0)) < _SPACE_CONTEXT_TTL_SECONDS:
        return cached
    return None


def _cache_space_context(
    space_id: str,
    *,
    devices: List[Any],
    ...
    record = {
        ...
        "scenes": scenes,
        "payload": _build_space_payload(space_id, devices, scenes),
    }
    with _CONTEXT_CACHE_LOCK:
        _SPACE_CONTEXT_CACHE[space_id] = record
    return record


def _invalidate_space_context(space_id: Optional[str] = None) -> None:
    with _CONTEXT_CACHE_LOCK:
        if space_id:
            _SPACE_CONTEXT_CACHE.pop(space_id, None)
            _SCENE_BUILDER_CACHE.pop(space_id, None)
        else:
            _SPACE_CONTEXT_CACHE.clear()
            _SCENE_BUILDER_CACHE.clear()


def _load_space_context(
    workflow: TuyaAutomationWorkflow,
    space_id: str,
    *,
    allow_stale: bool = True,
) -> Dict[str, Any]:
    cached = _get_cached_space_context(space_id)
    if cached:
        return cached

    with _CONTEXT_CACHE_LOCK:
        stale = dict(_SPACE_CONTEXT_CACHE.get(space_id) or {})
...
def prewarm_tuya_caches(space_id: Optional[str]) -> None:
    ...
    try:
        workflow = _get_shared_workflow()
        _load_space_context(workflow, space_id)
...
def _get_scene_builder_context(
    ...
    space_context = _load_space_context(workflow, space_id)
    devices = space_context.get("devices") or []
    properties = space_context.get("properties") or {}
    scenes = space_context.get("scenes") or []

    devices_payload = [
        {
            "id": device.id,
            "friendly_name": (device.customName or device.name or "").strip() or "Device",
...
def describe_space(space_id: str, *, config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    context = _load_space_context(workflow, space_id)
    return context.get("payload") or _build_space_payload(space_id, [], [])


def inspect_device(device_id: str, *, codes: Optional[Iterable[str]] = None) -> Dict[str, Any]:
    workflow, _ = _build_workflow()
    cached_properties: Dict[str, Dict[str, Any]] = {}
    default_space = os.getenv("TUYA_SPACE_ID")
    if default_space:
        cached_ctx = _get_cached_space_context(default_space)
        if cached_ctx:
            cached_properties = cached_ctx.get("properties", {})

    if device_id in cached_properties and not codes:
        prop_objects = cached_properties.get(device_id, {})
    else:
...
def propose_automation(
    ...
    *,
    ...
    space_context = _load_space_context(workflow, space_id)
    device_map = space_context.get("device_map") or {}
    properties_map = space_context.get("properties") or {}
    proposals, device_map, properties = _prepare_proposals(
        workflow,
        config,
        space_id,
        heuristic_keys,
...
def create_and_enable_automation(
    ...
    *,
    ...
    _invalidate_space_context(payload.get("space_id"))
...
def delete_automations(
    ...
    *,
    ...
    _invalidate_space_context(resolved_space)
...
def _prepare_proposals(
    ...
    *,
    ...
    needs_fetch = properties is None or not all(dev in properties for dev in device_ids)
    if needs_fetch:
        properties = workflow.inspect_properties(device_ids)
    else:
        properties = {dev: props for dev, props in properties.items() if dev in device_ids}

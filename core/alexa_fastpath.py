...
def _has_fastpath_keywords(normalized_input: str) -> bool:
    ...
    if not any(term in normalized_input for term in required_terms):
        return False
    if "bateria" not in normalized_input and "battery" not in normalized_input:
        return False
    if not any(
...
def _extract_action(normalized_input: str) -> Optional[bool]:
    turn_on_keywords = ("liga", "ligar", "ativa", "ativar", "aciona", "acionar", "start", "turn on", "enable")
    turn_off_keywords = ("desliga", "desligar", "desativa", "desativar", "apaga", "apagar", "stop", "turn off", "disable")
...
def _compose_rule_name(*, action_on: bool, device_name: str, comparator: str, threshold: int) -> str:
    action = "Turn on" if action_on else "Turn off"
    comparator_label = {
        ">": "above",
        "<": "below",
        "==": "equal to",
    }.get(comparator, "above")
    return f"{action} {device_name} when battery is {comparator_label} {threshold}%"
...
def _build_success_message(
    *,
    ...
    action_phrase = "turn on" if action_on else "turn off"
    comparator_phrase = {
        ">": "above",
        "<": "below",
        "==": "equal to",
    ...
    return (
        f"All set! Automation '{rule_name}' was created and enabled. "
        f"I will {action_phrase} {device_name} when the battery is {comparator_phrase} {threshold}%."
    )

...
def _format_short_report(stats: Dict[str, Any]) -> str:
    ...
    return (
        "Usage report (last 7 days, minute by minute)\n"
        f"- Avg SOC by hour: synthesized for trends (not exact values).\n"
        f"- Hours with highest solar generation: {hh(stats.get('best_solar_hours') or [])}.\n"
        f"- Hours with lowest SOC: {hh(stats.get('lowest_soc_hours') or [])}.\n"
        f"- Overall average SOC: {stats.get('overall_avg_soc')}%.\n"
        f"- Overall average solar generation: {stats.get('overall_avg_solar')} W.\n"
        "Prioritize heavy loads during high-solar windows; avoid deep discharges ahead of solar peaks; "
        "schedule delayed loads when solar is typically stronger."
    )

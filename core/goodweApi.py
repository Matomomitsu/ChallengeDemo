...
class GoodweApi:
    ...
    def ChangeEvChargerChargingMode(
        ...
        """
        Update the EV charger charge_mode for the provided powerstation_id.
        """
        token = self.GetToken()
        if not token:
            return {"hasError": True, "msg": "No token"}
        ...
        if updated:
            return {"hasError": False, "msg": "Charging mode updated successfully"}
        else:
            return {"hasError": True, "msg": "Powerstation not found"}
